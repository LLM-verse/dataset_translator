...
class DataParser(metaclass=ForceBaseCallMeta):
    def __init__(self, file_path: str,
        ...
        if self.do_translate:
            ...
            assert set(self.target_fields).issubset(set(self.target_config)), \
                f"The target fields {self.target_fields} do not exist in the target config {self.target_config}"
    ...
    def validate(self, keys: List[str]) -> bool:
        dict_fields = self.target_config
        for key in dict_fields:
            assert key in keys, f"\n Invalid parser, the key '{key}' is missing from {dict_fields}\n" \
                                f"you can adjust the fields {self.target_config} in the 'configs/*.py'" \
